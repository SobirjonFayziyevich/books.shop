...
class Community {
  ...
  async saveArticleData(data) {
    //faqat save maqsadda ishlatiladi bu mathodimiz.
    try {
  ...
  async getMemberArticlesData(member, mb_id, inquiry) {
    ...
    try {
      ...
      const result = await this.boArticleModel
        .aggregate([
          // aggregateni ichiga arrayni provite qildim.
  ...
  async getArticlesData(member, inquiry) {
    try {
      ...
      let matches =
        inquiry.bo_id === "all" // matches objectini hosil qilib oldim va inquiry ichidagi bo_idni qiymatini (all)ga teng bulsa,
  ...
  async getChosenArticleData(member, art_id) {
    try {
      ...
      if (member) {
        // agar men login bulmaga user bulsam, buyerga malumot kelmaydi.
